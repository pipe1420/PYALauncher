                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            softwareList.Add(new Software
                            {
                                ...
                                Machines = reader.IsDBNull(16) ? null : JsonConvert.DeserializeObject<List<string>>(reader.GetString(16))
                            });
                        }
                    }
                }
            ...
            try
            {
                using (var connection = new NpgsqlConnection(_connectionString))
                {
                        ...
                        "machines = EXCLUDED.machines;", connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@machines", JsonConvert.SerializeObject(software.Machines));
